...
void Game::restart() {
	...
	for (int y = 0; y < START_SIZE; y++) {
		_Vec2.push_back({startX, startY + y});
	}
}
...
void Game::run() {
	...
	while (_dynGuiManager.nibblerGui->input.quit == false) {
		...
		// draw on screen
		_dynGuiManager.nibblerGui->draw(_Vec2);
...
void Game::_move(Direction::Enum direction) {
	...
	if (_Vec2.size() > 0) {
		Vec2 newVec2(_Vec2[0].x + addX, _Vec2[0].y + addY);
...
void Game::_update() {
	...
	// update win
	if (_Vec2.size() >= _gameInfo->boardSize * _gameInfo->boardSize) {
		_gameInfo->win = true;
	}
	// update gameOver
	if (_Vec2.size() == 0) {
	...
	else if (_Vec2[0].x < 0 || _Vec2[0].x >= _gameInfo->boardSize
	|| _Vec2[0].y < 0 || _Vec2[0].y >= _gameInfo->boardSize) {
		_gameInfo->gameOver = true;
	}
	auto it = std::find(++_Vec2.begin(), _Vec2.end(), _Vec2[0]);
	if (it != _Vec2.end()) {
		_gameInfo->gameOver = true;
	}
	...
	if (_gameInfo->direction != _dynGuiManager.nibblerGui->input.direction) {
		if (_Vec2.size() <= 1) {
			...
			Vec2 direction(_Vec2[0].x - _Vec2[1].x, _Vec2[0].y - _Vec2[1].y);
